...
class ABCNN(nn.Module):
    ...
    def __init__(self, filter_channel=100, layer_size=1, match='cosine', inception=True):
...
class InceptionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        out_2 = self.conv_2(x)
        out_3 = self.conv_3(x)
        max_size = max(out_1.size(1), out_2.size(1), out_3.size(1))

        out_3 = F.pad(out_3, (0, 0, 0, max_size - out_3.size(1)))
